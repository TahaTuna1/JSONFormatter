...
def filter_json(json_data):
    ...
    if "data" in data:
        for currency, currency_data in data["data"].items():
            new_data[currency] = {
                'name': currency_data.get('name', None),
                'symbol_native': currency_data.get('symbol_native', None),
                'code': currency_data.get('code', None),
            }

    final_dict = {"data": new_data}

    # Convert the new_data back to JSON string and return
    return json.dumps(final_dict, indent=4)


# JSON data to be filtered
json_data = {
  "data": {
    "AED": {
      "symbol": "AED",
...
filtered_data = filter_json(json_data)
print(filtered_data)
